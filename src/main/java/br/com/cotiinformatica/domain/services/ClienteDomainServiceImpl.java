...
public class ClienteDomainServiceImpl implements ClienteDomainService {
	...
	@Override
	public CriarClienteResponseDto criarCliente
		...
		cliente.setSenha(sha256Component
				.criptografarSHA256(dto.getSenha()));
		...
		CriarClienteResponseDto response = modelMapper.map
				(cliente, CriarClienteResponseDto.class);
	...
	@Override
	public AutenticarClienteResponseDto autenticarCliente
		(AutenticarClienteRequestDto dto) {
		
		// buscar o cliente no bando de dados
		Cliente cliente = clienteRepository.findByEmailAndSenha
				(dto.getEmail(), sha256Component.
						criptografarSHA256(dto.getSenha()));
		...
		if(cliente != null) {
			...
			AutenticarClienteResponseDto response = modelMapper.map
					(cliente, AutenticarClienteResponseDto.class);
			response.setDataHoraAcesso(new Date());
			
			try {
				response.setToken(tokenComponent.generateToken
						(cliente.getEmail()));
			} catch(Exception e) {
				e.printStackTrace();
			}

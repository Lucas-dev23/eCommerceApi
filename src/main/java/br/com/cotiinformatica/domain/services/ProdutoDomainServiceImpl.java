...
public class ProdutoDomainServiceImpl implements ProdutoDomainService{
	...
	public CriarProdutoResponseDto criarProduto(CriarProdutoRequestDto dto) {
		...
		CriarProdutoResponseDto response 
			= modelMapper.map(produto, CriarProdutoResponseDto.class);
	...
	public List<ConsultarProdutosResponseDto> consultarProdutos() {
		...
		List<ConsultarProdutosResponseDto> response
			= modelMapper.map(produtos, new TypeToken
					<List<ConsultarProdutosResponseDto>>()
					{}.getType());
		return response;
	}
	...
	public ConsultarProdutosResponseDto obterProduto(UUID id) {
		...
		if(produto.isPresent()) {
			...
			ConsultarProdutosResponseDto response
				= modelMapper.map(produto,
						ConsultarProdutosResponseDto.class);
			return response;
		}
		else {
			return null;
